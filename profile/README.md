...
> ＯＵＲ　ＳＯＬＵＴＩＯＮ：Our solution involves implementing IAM accounts for all team members in accordance with AWS best practices. To ensure the security of our client, Cyber Fortress has decided to implement a Virtual Private Cloud (VPC), subnetting the network into a private and public subnet. Windows Server DC will be hosted on a private subnet of a VPC. The server will be CIS-compliant and Cyber Fortress will ensure server hardening and data protection on the server. Data will be encrypted at rest and encrypted in transit. In addition, Cyber Fortress will setup a Linux server instance containing PII and PCI data, ensuring CIS-compliance. For continuous security, Cyber Fortress will monitor event logs in real time from key assets including EC2 intances. Cyber Fortress will demonstrate an attack on the cloud in
